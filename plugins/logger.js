...
function isEqualOrHigherLevelBase(current_log_level, log_level) {
  const current = LOG_LEVELS[current_log_level]
  const checking = LOG_LEVELS[log_level]
    ...
    try {
      log_levels.forEach(level => {
        if (isEqualOrHigherLevelBase(level, log_level))
          this.appendToFile(
            `[${acronymizeLevel(log_level)}] ${text}`,
            PROTOCOL_FILES[level]
          )
      })
    } catch (e) {
